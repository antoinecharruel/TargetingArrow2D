...
func process_arrow(target_position: Vector2) -> void:
	...
	for i in range(total_nodes):
		...
		_apply_node_scale(i, bezier_positions, t_scale)


func _apply_node_scale(i: int, bezier_positions: Array, t_scale: float):
		...
		if i == 0 and total_nodes > 1: # Head
			var dir: Vector2 = bezier_positions[1] - bezier_positions[0]
			var head_angle: float = signed_angle(Vector2.DOWN, dir)
			_nodes[i].rotation_degrees = head_angle
			
			var angle_rad: float = deg_to_rad(head_angle)
			var rotated_offset: Vector2 = head_texture_offset.rotated(angle_rad)
			_nodes[i].global_position = bezier_positions[0] - rotated_offset * _nodes[i].scale
		
		else: # Nodes
			var dir: Vector2 = bezier_positions[i] - bezier_positions[i - 1]
			_nodes[i].rotation_degrees = signed_angle(Vector2.DOWN, dir)
			...
			if cumulative_dist < cumulative_dist_max:
				...
				_nodes[i].global_position = bezier_positions[i]
